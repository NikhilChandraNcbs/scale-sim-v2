...
def plot_storage_comparison(layers, dense_storage, sparse_filter, sparse_metadata, title):
    ...
    # Plot the dense storage bar
    # print("index and storage")
    # print(index)
    # print(dense_storage)
    ax.bar(index, dense_storage, bar_width, label='Dense Storage', color='b')
    
    # Plot the sparse filter and sparse metadata as stacked bars
    ax.bar(index + bar_width, sparse_filter, bar_width, label='Sparse Filter Storage', color='g')
    ax.bar(index + bar_width, sparse_metadata, bar_width, bottom=sparse_filter, label='Sparse Metadata Storage', color='r')
    
    # Labels, title, legend
    ax.set_xlabel('Layers')
    ax.set_ylabel('Memory Size (Words)')
    ax.set_title(f'{title}: Memory Storage Comparison: Dense vs Sparse')
...
model = 'resnet18'
size = '1mb'
sparsities = ['1s4', '2s4']  # Multiple sparsities
...
for sparsity in sparsities:
    # Read the storage values from SPARSE_REPORT.csv
    dense_storage, sparse_filter, sparse_metadata = read_sparse_report(model, size, sparsity)
    
    if dense_storage is not None:
        # Plot the storage comparison for each sparsity
        plot_storage_comparison(layers, dense_storage, sparse_filter, sparse_metadata, f'{model}_{sparsity}')
'''

# Plots all sparsities in the same graph
...
def read_sparse_report(model, size, sparsity):
    # file_path = f'sparsity_results/{model}_{size}_{sparsity}/SPARSE_REPORT.csv'
    file_path = f'sparsity_results_128x128/{model}_{size}_{sparsity}_128x128/SPARSE_REPORT.csv'
...
def plot_combined_storage_style1(layers, dense_storage, sparse_filter_1s4, sparse_metadata_1s4, sparse_filter_2s4, sparse_metadata_2s4, sparse_filter_3s4, sparse_metadata_3s4, title_prefix):
    ...
    ax.set_ylabel('Memory Size (Words)', fontsize=14)
    ax.set_title(f'{title_prefix}: Memory Storage Comparison: Dense vs 1:4 vs 2:4 vs 3:4')
    ax.set_xticks(index + bar_width)
    ax.set_xticklabels(layers, rotation=45, ha="right", fontsize=12)
    ax.legend()
...
def plot_combined_storage_style2(layers, dense_storage, sparse_filter_1s4, sparse_metadata_1s4, sparse_filter_2s4, sparse_metadata_2s4, title_prefix):
    ...
    plt.tight_layout()
    plt.savefig('memoryplot2.png', dpi=300, bbox_inches='tight', format='png')
    # plt.show()
...
# plot_combined_storage_style1(layers, dense_storage, sparse_filter_1s4, sparse_metadata_1s4, sparse_filter_2s4, sparse_metadata_2s4, model)
